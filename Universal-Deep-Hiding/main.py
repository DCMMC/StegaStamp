...
def main():
    ...
    if opt.test == '':
        ...
        # (DCMMC) we only need ImageNet (ILSVRC2012) validation images
        testdir = valdir
    ...
    # (DCMMC) for DDH (trained with train_mani_ddh.sh)
    ...
    if opt.test == '':
     ...
     # For testing the trained network
        ...
        # (DCMMC) secret and cover come both from ImageNet Val
...
def forward_pass(secret_img, secret_target, cover_img, cover_target, Hnet, Rnet, criterion, val_cover=0, i_c=None, position=None, Se_two=None):
    # (DCMMC) secret_target and cover_target are not used
    ...
    # (DCMMC) cover with secret as blind watermark
    itm_secret_img = Hnet(H_input)
    if i_c !=None:
        print('DCMMC: i_c != None')
    ...
    # (DCMMC) measure how "blind"
    errH = criterion(container_img, cover_imgv)  # Hiding net

    # (DCMMC) get the watermark from the watermarked picture
...
def save_result_pic_analysis(bs_secret_times_num_training, cover, container, secret, rev_secret, epoch, i, save_path=None, postname=''):
    ...
    # (DCMMC) change 4 to opt.num_cover
    for i_cover in range(opt.num_cover):
        # print('DCMMC: i_cover*channel_cover={}'.format(i_cover * opt.channel_cover))
        ...
        if i_cover == 0:
            ...
            # print("DCMMC: showCover={}".format(showCover.shape))
        else:
            # print('DCMMC: showCover={}, cover_i={}, container_i={}, cover_gap_i={}'.format(
            #     showCover.shape, cover_i.shape, container_i.shape, cover_gap_i.shape))
            showCover = torch.cat((showCover, cover_i, container_i, cover_gap_i),0)

    # (DCMMC) change 4 to opt.num_cover
    for i_secret in range(opt.num_cover):
