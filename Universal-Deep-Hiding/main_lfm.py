...
def analysis(val_loader, epoch, Hnet, Rnet, criterion):
    ...
    img = torch.tensor(cv2.resize(img, (128, 128)), dtype=torch.float)
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(val_loader, 0):
        # (DCMMC) qr code as secret
        secret_img = np.transpose(secret_img, (0, 2, 3, 1))
        for idx in range(secret_img.shape[0]):
            secret_img[idx] = img
...
def save_result_pic(bs_secret_times_num_training, cover, container, secret, rev_secret, epoch, i, save_path=None, postname=''):
    ...
    resultImgName = '%s/ResultPicsLFM_epoch%03d_batch%04d%s.png' % (save_path, epoch, i, postname)
